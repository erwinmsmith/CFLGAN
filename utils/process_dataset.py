...
def process_sft_dataset(dataset_name, dataset, dataset_sample):
    if dataset_name in ["lucasmccabe-lmi/CodeAlpaca-20k", "yahma/alpaca-cleaned", "FinGPT/fingpt-sentiment-train","pubgen"]:
        ...
        dataset = dataset.rename_column("output", "response")
    elif dataset_name in ['alpaca']:       # TODO: 'lavita/ChatDoctor-HealthCareMagic-100k'. not sure whether to discard the instruction.
        dataset = dataset.remove_columns(['input'])
        #dataset = dataset.rename_column("instruction", "instruction")
        dataset = dataset.rename_column("output", "response")
    else:
